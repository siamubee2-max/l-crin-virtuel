...
export default function Gallery() {
  ...
  return (
    <div className="space-y-12">
      {/* Hero Section */}
      <section className="relative rounded-3xl overflow-hidden bg-neutral-900 border border-white/10 text-white p-8 md:p-16 flex flex-col md:flex-row items-center justify-between gap-8 shadow-2xl shadow-black/50">
        <div className="absolute inset-0 opacity-30 bg-[url('https://images.unsplash.com/photo-1573408301185-9146fe634ad0?q=80&w=2075&auto=format&fit=crop')] bg-cover bg-center mix-blend-overlay" />
        <div className="relative z-10 max-w-xl space-y-6">
          <h1 className="text-4xl md:text-5xl font-serif leading-tight text-white drop-shadow-lg">
      ...
      <section>
        <div className="flex justify-between items-end mb-8">
          <div>
            <h2 className="text-3xl font-serif text-white">{t.gallery.myCreations}</h2>
            <p className="text-stone-400 mt-2">{t.gallery.latestTryons}</p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {creations?.map((creation, idx) => (
              <motion.div
                ...
                className="group bg-neutral-900 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl hover:shadow-amber-900/10 transition-all duration-300 border border-white/5 flex flex-col"
              >
                {/* Image Container */}
                <div className="relative aspect-[4/5] overflow-hidden bg-neutral-800">
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4">
                     <div className="w-16 h-16 rounded-lg overflow-hidden border border-white/20 bg-neutral-800">
                ...
                {/* Card Footer */}
                <div className="p-5 flex justify-between items-center bg-neutral-900 z-10 border-t border-white/5">
                  <div>
                    <p className="font-medium text-white capitalize font-serif tracking-wide">
                  ...
                  <div className="flex gap-2">
                    <Button variant="ghost" size="icon" onClick={() => window.open(creation.result_image_url, '_blank')} className="text-stone-400 hover:text-white hover:bg-neutral-800">
